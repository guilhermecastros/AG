...
class PSO(object):
  ...
  def __init__(self, func, particle_dim, n_particles, init_space, end_space, XX, w):
    ...
    #np.random.uniform(init_space, end_space, size=(n_particles, self.particle_dim))
  ...
  def optimize(self, maxiter=300):
    ...
    for _ in range(maxiter):
      for i in range(self.n_particles):
          ...
          self.velocities[i] = self.update_velocity(x, v, p_best, self.g_best, XX=self.XX, w=self.w)
...
if __name__ == '__main__':
  ...
  for i in range(31):
    ...
    PSO_s = PSO(func=sphere, particle_dim=3, n_particles=n_particles, init_space=-100, end_space=100, XX=0.5, w=0.5)
